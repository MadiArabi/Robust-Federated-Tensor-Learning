...
from tensorly.base import partial_tensor_to_vec
...
class TuckerRegressor:
    ...
    #rng = .check_random_state(self.random_state)
        ...
        G = T.tensor(self.G, **T.context(X))
        W = []
        for i in range(1, T.ndim(X)):  # First dimension of X = number of samples
            W.append(T.tensor(self.U[i-1],**T.context(X)))
        ...
        for iteration in range(self.n_iter_max):
            ...
            for i in range(len(W)):
                #print("Shape of kronecker(W, skip_matrix=i):", kronecker(W, skip_matrix=i).shape)
                #print("Shape of transpose(unfold(G, i)):", T.transpose(unfold(G, i)).shape)
