...
- [x] ~~Run §3.5.1 sample-level contamination pilot to confirm motivation~~ — done on real data (not the Chapter 2 simulator); see `experiment_log.md`. Motivation held up via subspace-angle degradation (motivation pilot), though downstream TTF prediction proved largely insensitive to physically-plausible contamination — see `direction1_byzantine_aggregation_findings.md` §3 and the broader Session 6–7 narrative for why, and what was tried instead.
...
- [ ] **New:** reconcile §3.2.2's RFTL-U (Grassmann-distance user trimming) with the empirical Byzantine-robust aggregation benchmark in `direction1_byzantine_aggregation_findings.md` — six aggregation rules (median, trimmed mean, Krum, Multi-Krum, geometric median, vs. plain sum) were benchmarked directly at the server-side scatter-matrix aggregation point, with Multi-Krum emerging as the clear winner. Decide whether this supersedes, extends (as a 7th candidate rule), or sits alongside the RFTL-U proposal as written.
- [ ] Pull and verify exact citations for Krum, coordinate-median/trimmed-mean, and the small-magnitude/"stealthy" Byzantine attack literature (referenced by mechanism, not yet confirmed by venue/year) — see `direction1_byzantine_aggregation_findings.md` §6.
